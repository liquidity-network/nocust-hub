...
from operator_api.util import red, ZERO_CHECKSUM


class TransactionMerkleTree:
    def __init__(self, transactions):
        self.transactions = normalize_transactions(transactions)
        self.merkle_tree_leaf_map = {}
        self.merkle_tree_nonce_map = {}
        self.root = calculate_transaction_merkle_tree(
            self.transactions, self.merkle_tree_leaf_map, self.merkle_tree_nonce_map, 0)
...
def calculate_transaction_merkle_tree(leaves, leaf_map, nonce_map, index):
    n = len(leaves)
    if n == 1:
        result = {
            'node': leaves[0],
    ...
    left = leaves[0:mid]
    right = leaves[mid:n]
    left_child = calculate_transaction_merkle_tree(
        left, leaf_map, nonce_map, 2 * index)
    right_child = calculate_transaction_merkle_tree(
